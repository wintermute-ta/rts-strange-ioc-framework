Shader "Custom/Terrain" {
	...
	SubShader {
		...
		float3 GetHexCoord(Input IN)
		{
			float outerRadius = 10.0;
			float outerToInner = 0.866025404;
			float innerRadius = outerRadius * outerToInner;
			
			float3 position = float3(IN.worldPos.x - 4.0, IN.worldPos.y, IN.worldPos.z - 4.0);
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float3 hexCoord = GetHexCoord(IN);
			if ((hexCoord.x == _Point.x) && (hexCoord.z == _Point.z))
			{
