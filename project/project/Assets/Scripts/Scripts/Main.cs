...
public class Main : MonoBehaviour
{
            ...
            if (hexGrid != null)
            {
                if (hexGrid.selectedCell != null)
                {
                    if (hexGrid.selectedCell.MapCell.IsSpawnCell)
                    {
                        if (!hexGrid.selectedCell.MapCell.IsWalkable())
                        {
                            RadialMenu menu = Instantiate(ResourceManager.Instance.GetPrefabComponent<RadialMenu>("RadialMenuWorld"));
                            List<RadialItemContext> context = new List<RadialItemContext>();
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_CREATE_GUN, CreateGun));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_DUMMY, DummyFunction));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_RETURN, DummyFunction));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_UPGRADE, DummyFunction));
                            menu.Init(context.ToArray(), HexMetrics.Perturb(hexGrid.selectedCell.Position));
                            menu.CloseAction = delegate { SetInteractable(); };
                        ...
                        else
                        {
                            RadialMenu menu = Instantiate(ResourceManager.Instance.GetPrefabComponent<RadialMenu>("RadialMenuWorld"));
                            List<RadialItemContext> context = new List<RadialItemContext>();
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_CREATE_SHIP, CreateShip));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_DUMMY, DummyFunction));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_RETURN, DummyFunction));
                            context.Add(new RadialItemContext(RadialItemContext.ActionType.ACTION_UPGRADE, DummyFunction));
                            menu.Init(context.ToArray(), HexMetrics.Perturb(hexGrid.selectedCell.Position));
    ...
    public void SetInteractable(bool b = true)
    {
        hexGrid.Interactable = b;
        interactable = b;
    }
