...
public class HexGridChunk : MonoBehaviour {
    ...
    public void RefreshHighlight(bool highlighted, HexCoordinates coordinates)
    {
        if (highlighted)
        {
            terrain.SetHighlightCoord(coordinates);
        }
    }
	...
	void Triangulate (HexDirection direction, HexGridCell cell) {
        ...
        if (cell.MapCell.IsSpawnCell || cell.MapCell.IsPathCell)
        {
    ...
    private void TriangulateHighlight(HexDirection direction, HexGridCell cell, Vector3 center)
    {
        ...
        color = cell.MapCell.IsPathCell ? Color.Lerp(color, Color.blue, 0.5f) : color;
        color = cell.Selected ? Color.Lerp(color, new Color(0.0f, 1.0f, 1.0f, 1.0f), 0.5f) : color;
    ...
    void TriangulateWater (
		HexDirection direction, HexGridCell cell, Vector3 center
	) {
	...
	void TriangulateOpenWater (
		HexDirection direction, HexMapCell cell, HexMapCell neighbor, Vector3 center
	) {
	...
	void TriangulateWaterShore (
		HexDirection direction, HexGridCell cell, HexGridCell neighbor, Vector3 center
	) {
		...
		if (nextNeighbor != null) {
			Vector3 v3 = nextNeighbor.Position + (nextNeighbor.MapCell.IsUnderwater ?
			...
			waterShore.AddTriangleUV(
				new Vector2(0f, 0f),
				new Vector2(0f, 1f),
				new Vector2(0f, nextNeighbor.MapCell.IsUnderwater ? 0f : 1f)
			);
		}
	}
	...
	void TriangulateWithoutRiver (
		HexDirection direction, HexMapCell cell, Vector3 center, EdgeVertices e
	) {
	...
	void TriangulateWithRiverBeginOrEnd (
		HexDirection direction, HexGridCell cell, Vector3 center, EdgeVertices e
	) {
		EdgeVertices m = new EdgeVertices(
			Vector3.Lerp(center, e.v1, 0.5f),
			Vector3.Lerp(center, e.v5, 0.5f)
		...
		TriangulateEdgeStrip(
			m, color1, cell.MapCell.TerrainTypeIndex,
			e, color1, cell.MapCell.TerrainTypeIndex
		);
	...
	void TriangulateConnection (
		HexDirection direction, HexGridCell cell, EdgeVertices e1
	) {
        HexGridCell neighbor = cell.GetNeighbor(direction);
		if (neighbor == null) {
			return;
			...
			if (!cell.MapCell.IsUnderwater) {
				if (!neighbor.MapCell.IsUnderwater) {
					TriangulateRiverQuad(
                        ...
                        cell.MapCell.HasIncomingRiver && cell.MapCell.IncomingRiver == direction
					);
				}
				...
				!neighbor.MapCell.IsUnderwater &&
				neighbor.MapCell.Elevation > cell.MapCell.WaterLevel
			) {
				TriangulateWaterfallInWater(
		...
		if (cell.MapCell.GetEdgeType(direction) == HexEdgeType.Slope) {
			...
			TriangulateEdgeStrip(
				e1, color1, cell.MapCell.TerrainTypeIndex,
				e2, color2, neighbor.MapCell.TerrainTypeIndex, hasRoad
			);
		}
		...
		if (direction <= HexDirection.E && nextNeighbor != null) {
			...
			if (cell.MapCell.Elevation <= neighbor.MapCell.Elevation) {
				if (cell.MapCell.Elevation <= nextNeighbor.MapCell.Elevation) {
					TriangulateCorner(
						e1.v5, cell.MapCell, e2.v5, neighbor.MapCell, v5, nextNeighbor.MapCell
                    );
				}
				else {
					TriangulateCorner(
						v5, nextNeighbor.MapCell, e1.v5, cell.MapCell, e2.v5, neighbor.MapCell
                    );
				}
			}
			else if (neighbor.MapCell.Elevation <= nextNeighbor.MapCell.Elevation) {
				TriangulateCorner(
					e2.v5, neighbor.MapCell, v5, nextNeighbor.MapCell, e1.v5, cell.MapCell
                );
			}
			else {
				TriangulateCorner(
	...
	void TriangulateCorner (
		Vector3 bottom, HexMapCell bottomCell,
		Vector3 left, HexMapCell leftCell,
		Vector3 right, HexMapCell rightCell
	) {
	...
	void TriangulateEdgeTerraces (
		EdgeVertices begin, HexMapCell beginCell,
		EdgeVertices end, HexMapCell endCell,
		bool hasRoad
	) {
	...
	void TriangulateCornerTerraces (
		Vector3 begin, HexMapCell beginCell,
		Vector3 left, HexMapCell leftCell,
		Vector3 right, HexMapCell rightCell
	) {
